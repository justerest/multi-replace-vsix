...
describe('class StringTransformerImp', () => {
  ...
  beforeEach(() => {
    stringTransformerImp = new StringTransformerImp(void 0, () => camelCase);
  });
  ...
  describe('method replaceText', () => {
    it('should replace preserving cases', () => {
      expect(stringTransformerImp.replace('searchValue', 'search_value', 'replace-value')).toBe(
        'replaceValue',
      );
    });
    ...
    it('should replace angular case', () => {
      expect(
        stringTransformerImp.replace('user.service', 'user service', 'currentUserService'),
      ).toBe('current-user.service');
    });

    it('should camelCase if not identify', () => {
      expect(stringTransformerImp.replace('user', 'user', 'currentUser')).toBe('currentUser');
    });

    it('should use default transformer if not identify', () => {
