...
export class StringTransformerImp {
  static defaultTransformer: () => CaseTransformer = () => camelCase;

  constructor(
    private caseTransformers: CaseTransformer[] = defaultTransformers,
    private defaultTransformer: () => CaseTransformer = () =>
      StringTransformerImp.defaultTransformer(),
  ) {}
  ...
  replace(sourceString: string, searchValue: string, replaceValue: string): string {
    ...
    return sourceString.replace(regExp, (matched) =>
      matched in replaceMap ? replaceMap[matched] : matched,
    );
  }
  ...
  private setDefaultTransformer() {
    const defaultTransformer = this.defaultTransformer();
    this.caseTransformers.sort((transformer) => (transformer === defaultTransformer ? 1 : -1));
  }
}
